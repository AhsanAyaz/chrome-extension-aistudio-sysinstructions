...
- [x] **PUSH-05
**: A `null`, missing, or empty-array localStorage read is never auto-propagated as "user deleted everything" — empty results are treated as a detection failure unless the user explicitly clicks Push Now.
- [x] **PUSH-06
**: Unknown fields on instruction items (anything beyond `title` and `text`) are preserved end-to-end so future AI Studio schema additions are not silently dropped.
